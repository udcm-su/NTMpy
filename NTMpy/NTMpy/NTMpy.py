...
class simulation(object): 
    ...
    def addSubstrate(self,name = "silicon"): 
        ...
        if (name == "Silicon") or (name =="silicon") or (name =="Si"):
            ...
            if self.num_of_temp == 2:#Lattice only in the 2T
                self.temp_data_Lat.addLayer(20e-9,k_lat_Si,C_lat_Si,rho_Si)
                self.coupling = np.append(self.coupling,G_Si)
                self.temp_data_Lat.addLayer(100e-9,k_lat_Si,C_lat_Si,rho_Si)
                self.coupling = np.append(self.coupling,G_Si)
                self.temp_data_Lat.addLayer(100000e-9,k_lat_Si,C_lat_Si,rho_Si)
                self.coupling = np.append(self.coupling,G_Si)
            #In the 1 and 2 temperature case electron always gets appended
            self.temp_data.addLayer(20e-9,k_el_Si,C_el_Si,rho_Si)
            self.temp_data.addLayer(100e-9,k_el_Si,C_el_Si,rho_Si)
            self.temp_data.addLayer(100000e-9,k_el_Si,C_el_Si,rho_Si)
            #Adjust the source and consider a multi layer Si-substrate
            #That is inject the refractive index of silicon after the last film layer
            #and before the air layer.
            if self.source.substratesetup == False:
                #We do changes in the source class, that is we have to 
                #make sure, that the ´n_vec´ does not always get appended more and more
                self.source.n_vec = np.concatenate((self.source.n_vec[:-1],\
                                                    [5.5674+0.38612j, 5.5674+0.38612j, 5.5674+0.38612j],\
                                                    [self.source.n_vec[-1]]))
                self.source.substratesetup = True
            
    def addLayer(self,L,conductivity,heatCapacity,rho,coupling=0,*args):
        """
        Add parameters of every layer: 
        (length,conductivity[electron,lattice,spin],heatCapacity[electron, lattice, spin],density, coupling[E-L,L-S,S-E])
        The units in SI are: 
        ...
        if self.num_of_temp == 3:
            if (np.size(conductivity) or np.size(heatCapacity) or np.size(coupling))<3: 
                ...
                return(128)
            self.temp_data_Lat.addLayer(L,conductivity[1],heatCapacity[1],rho)
            self.temp_data_Spin.addLayer(L,conductivity[2],heatCapacity[2],rho)
    ...
    def run(self):
        ...
        if self.temp_data_Spin:
            ...
            print('-----------------------------------------------------------')  
            return(phi_E,phi_L,phi_S,x_plt_flat,t)
    #=======End 3 temp Case =================================
...
class visual(object): 
    ...
    def localAbsorption(self): 
        ...
        #Calculating the 1D absorption profile according to TMM
        [absorption,grid] = self.so.absorption(self.so.theta_in,self.so.lambda_vac,self.so.n_vec,d_vec,self.so.polarization,np.shape(xflat)[0])
        [M,M_n,trans,r,T,R,A,theta] = self.so.TM(self.so.theta_in,self.so.lambda_vac,self.so.n_vec,d_vec,self.so.polarization)
        
        plt.figure()
        plt.suptitle("Local absorption profile",fontsize = 12)
        plt.title("Total absorption = {abso:.2f}".format(abso = A ),fontsize=10)
        plt.xlabel("Distance from surface")
        plt.ylabel("Absorption per unit incident power")
    ...
    def contour(self,*args):
        ...
        if args:
            plt.title(r'Temperature of '+str(name)+' $T(x,t)$ in K')    
        plt.show()
    ...
    def average(self):
        ...
        if self.sim.num_of_temp == 3:#electron lattice and spin temperature under consideration
            ...
            if np.mean(tps) < 1e-9: 
                ...
                plt.ylabel(r'Temperature in K')
            plt.plot(tps,avT_S,c = 'b',label = 'Spin')
            plt.plot(tps,avT_L,c = 'k',label = 'Lattice')
            plt.plot(tps,avT_E,c = 'r',label = 'Electron')
            plt.title('Temperature averaged in space vs time')
            plt.grid()
            plt.legend()
            plt.show()
            return(self.t,avT_E,avT_L,avT_S)
    ...
    def animation(self,speed,sve = 0): 
        ...
        if self.sim.num_of_temp == 3:
            ...
            lineS, = ax.plot([],[],'g',animated = True,label = 'Spin Temp. in [K]')
