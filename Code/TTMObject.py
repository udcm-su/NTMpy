...
This is the object file of the Three temperature solver
1)Multi layers and 3 temperatures can be solved. 
2)The timestep for the euler loop is automatically calculated.
3)We use an approch of B-splines and explicit euler to compute the solution in space and time
...
Recent updates (05.06.2019)
i)  The solver is now capable of coupling 3 differential equations (Electron, Lattice, Spin)
    All of them are related to linear coupling constants
ii) The all of the systems can now be initialized individually. That is they
    do not have to have the same temperature at t=0. 
    This is used, i.e. for Transient Grating.
iii)Printing statements have been changed from print(f"...") to print("...".format())
    such that also Python <3.6 can use the package.
    
The package still has to be tested. 
    ...
    def average(self):
        ...
        if self.sim.num_of_temp == 3:#electron lattice and spin temperature under consideration
            ...
            if np.mean(self.t) < 1e-10: 
                ...
                plt.ylabel(r'Temperature in K')
            plt.plot(self.t,avT_S,c = 'b',label = 'Spin')
